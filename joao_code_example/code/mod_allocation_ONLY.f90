...
program allocation !module to test allocation logic module of LPJmL-Fire
    ...
    real(r_8), dimension(npls) :: test_bminc = 10 !kgC/m2
    ...
    do pls = 1,npls
        ...
        ! bminc_ind(pls) = ((bminc(pls)/nind(pls))*1.D3)    -  *original

        bminc_ind(pls) = ((test_bminc(pls)/nind(pls))*1.D3)
        !print*, 'BMINC (g/ind)', bminc_ind(pls)

        ! ====== TREE ALLOCATION ======

        lm1(pls) = (latosa*sm(pls))/((dwood(pls)*1000000)*height(pls)*sla(pls))  !allometric leaf mass requirement *****ATENÇÃO*****
        ! print*, 'LM1', lm1(pls)
        ...
        print*, 'LM (kgC/m²)', lm_ind(pls), 'RM', rm_ind(pls), 'SM', sm_ind(pls), 'HM', hm_ind(pls), pls
